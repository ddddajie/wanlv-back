...
-- ----------------------------
-- Table structure for sys_normal_user_refresh_token
-- ----------------------------
DROP TABLE IF EXISTS `sys_normal_user_refresh_token`;
CREATE TABLE `sys_normal_user_refresh_token`  (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  ...
  CONSTRAINT `fk_refresh_token_normal_user` FOREIGN KEY (`user_id`) REFERENCES `sys_normal_user` (`id`) ON DELETE CASCADE ON UPDATE RESTRICT
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '普通用户刷新令牌表' ROW_FORMAT = DYNAMIC;
...
-- ----------------------------
-- Table structure for agent_route_geo
-- ----------------------------
DROP TABLE IF EXISTS `agent_route_geo`;
CREATE TABLE `agent_route_geo`  (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
